...
const ALERT_TMP_DIR = '.sentinel-tmp';
const ALERT_QUEUE_DIR = '.sentinel-queue';
...
function buildCommandEnv(envelope) {
  ...
  const env = {
    ...process.env,
    SENTINEL_FILE: envelope.file,
    SENTINEL_EVENT_ID: envelope.event_id,
    SENTINEL_BATCH_ID: envelope.batch_id,
    SENTINEL_SCHEMA_VERSION: envelope.schema_version,
  };

  if (json.length <= MAX_ENV_CHANGES_CHARS) {
    env.SENTINEL_CHANGES = json;
    return env;
  }
  ...
  try {
    ...
    env.SENTINEL_CHANGES_FILE = outPath;
    env.SENTINEL_CHANGES = '[]';
    return env;
  } catch (err) {
    env.SENTINEL_CHANGES = '[]';
    env.SENTINEL_CHANGES_TRUNCATED = '1';
...
export async function postWebhook(url, envelope, options = {}) {
  ...
  const headers = {
    ...
    'X-Sentinel-Event-Id': envelope.event_id,
    'X-Sentinel-Batch-Id': envelope.batch_id,
    'X-Sentinel-Schema': envelope.schema_version,
    ...(options.headers ?? {}),
  };
