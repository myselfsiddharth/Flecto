...
const FULL = [
  ...
  record('src/explain.js', { lines: [100, 95], branches: [20, 19], functions: [10, 10] }),
...
describe('coverage report', () => {
  test('separates the security-relevant modules from everything else', () => {
    ...
    for (const file of ['config.js', 'policy.js', 'secrets.js', 'encrypted.js', 'pr-comment.js', 'pr-providers.js', 'snapshot-store.js', 'explain.js']) {
  ...
  test('orders each table worst branch coverage first', () => {
    ...
    assert.deepEqual(order, ['config', 'snapshot-store', 'pr-providers', 'pr-comment', 'encrypted', 'policy', 'explain', 'secrets']);
  });
