...
- **Custom policy packs are still code you are choosing to run.** Their
  regular expressions can no longer hang the process -- since 4.0 a pack
  outside `src/packs/` compiles its `match.path`, `afterMatches`, and
  `afterAnyMatches` with RE2, whose matching is linear in the length of the
  input, so catastrophic backtracking is not expressible. What a pack can still
  do is assert things you did not intend: define rules that never fire, or fire
  on everything, and so shape the gate. Review packs before enabling them, the
  same as any code you run in CI.
